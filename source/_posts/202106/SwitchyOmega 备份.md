title: SwitchyOmega 备份
date: '2021-06-22 20:39:55'
updated: '2021-06-22 20:40:25'
category: 
 - 代理
tags: [SwitchyOmega, proxy, 备份]
permalink: /articles/2021/06/22/1624365595584.html
---

文件路径：<https://www.biuaxia.cn/OmegaOptions.bak>

编辑源代码：

```javascript
[SwitchyOmega Conditions]
@with result

*.uzhuangji.net +proxy
*.tongyongpe.net +proxy
*.gfx.ms +proxy
*.msecnd.net +proxy
*.pinterest.com +proxy
*.tumblr.com +proxy
*.bootstrapcdn.com +proxy
*.typekit.net +proxy
*.raw.githubusercontent.com +proxy
*.cloudflare.com +proxy
*.cokemv.cn +direct
*.facebook.net +proxy
*.adsrvr.org +proxy
*.twitter.com +proxy
*.quora.com +proxy
*.t.co +proxy
*.jwt.io +proxy
*.typography.com +proxy
*.aspnetcdn.com +proxy
*.demdex.net +proxy
*.microsoft.com +proxy
*.imooc.com +direct
*.maps.googleapis.com +proxy
*.imaegoo.com +proxy
*.github.io +proxy
*.gstatic.com +proxy
*.fontawesome.com +proxy
*.amgdgt.com +proxy
*.google.com.tw +proxy
*.jsdelivr.net +proxy
*.d33wubrfki0l68.cloudfront.net +proxy
*.springboot.plus +proxy
*.dka575ofm4ao0.cloudfront.net +proxy
*.biuaxia.cn +direct
47.92.153.34 +direct
*.github.com +proxy
*.mhexo.github.io +proxy
*.choosealicense.online +proxy
*.bilibili.com +direct
*.google.co.jp +proxy
*.typescriptlang.org +proxy
*.polyfill.io +proxy
*.sharepointonline.com +proxy
*.cxyzjd.com +proxy
*.juejin.cn +direct
*.doubleclick.net +proxy
*.googleadservices.com +proxy
*.rlcdn.com +proxy
*.cdn.ink +proxy
*.addthis.com +proxy
*.google.com +proxy
*.google.ru +proxy
*.mapbox.com +direct
*.stackoverflow.com +proxy
*.stripe.com +proxy
*.jetbrains.com +proxy
*.newtab +direct
*.githubassets.com +proxy
*.acgur.com +proxy
*.golang.org +proxy
*.apache.org +proxy
*.93dd4aa065069a75.com +proxy
*.camo.githubusercontent.com +proxy
*.avatars.githubusercontent.com +proxy
*.mvnrepository.com +proxy
*.twimg.com +proxy
*.chromium.org +proxy
*.nyaa.si +proxy
*.adsco.re +proxy
*.advinci.uno +proxy
*.blogger.com +proxy
*.bp.blogspot.com +proxy
*.javx01.blogspot.com +proxy
*.eyny.com +proxy
*.cnxalex.blogspot.com +proxy
*.zimuku.la +proxy
*.reimu.net +proxy
*.novaopcj.top +proxy
*.optimizely.com +proxy
*.rawgit.com +proxy
*.gtimg.com +direct
*.qq.com +direct
*.telegram.org +proxy
*.v2ray.com +proxy
*.apkfab.com +proxy
*.wikipedia.org +proxy
*.vercel.com +proxy
*.telegra.ph +proxy
*.googlevideo.com +proxy
*.ytimg.com +proxy
*.ggpht.com +proxy
*.youtube.com +proxy
*.facebook.com +proxy
*.sstatic.net +proxy
*.imgur.com +proxy
*.tencent-cloud.com +direct
*.ajax.googleapis.com +proxy
*.v2ex.com +proxy
211.83.241.125 +direct
*.atlassian.net +proxy
*.netsarang.com +proxy
*.googleusercontent.com +proxy
*.gravatar.com +proxy
*.t.me +proxy
apkfab.com +proxy

* +direct

```
